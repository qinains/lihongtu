<%@ page language="C#" autoeventwireup="true" inherits="CustInfoManager_SetAuthenPhone, App_Web_setauthenphone.aspx.8268bb4f" enableEventValidation="false" %>
...
<body>
    ...
    <div class="findPasswordBox">
        ...
        <label for="checkCode">
            <span>验证码：</span><input type="text" id="checkCode" class="input" />
            <div id="sendCode" onclick="sendPhoneCode()">发送验证码</div>
            <em>*</em><p id="hintCode">请输入收到的验证码</p>
        </label>
        <input type="button" value="下一步" id="btnSave" onclick="checkSubmit()"/><img id="loadImg" src="Images/Loading.gif" style="display:none" />
        <p id="successNote">
    ...
    <script type="text/javascript">
        ...
        doCheckPhone = function(){
            ...
            if(!regMobile.test(phoneNum)){	//手机号格式不正确
			    $("#hintMobile").text("输入的手机不正确，请重新输入");		
			    $("#hintMobile").attr("class","hintError");
			    return false;
		    }else{
		        $("#hintMobile").text("");		
        ...
        sendPhoneCode = function(){
            if(doCheckPhone() == false){
			    ...
			    $.ajax({
			        ...
			        dataType:"json",
			        data:{CustID:custid,PhoneNum:phoneNum,Type:"authen",time:time.getTime()},
			        success:function(data,textStatus){
			            $.each(data,function(index,item){
			                if(item["result"]=="true"){
			                    $("#hintMobile").text("验证码已发送手机，请查收");		
			                    $("#hintMobile").attr("class","hintCorrect");
                                $("#hdCustID").val(item["custid"]);
                                startInterval();
                                alert(item["authencode"]);
			                }else if(item["result"]=="IsYours"){
			                    $("#hintMobile").text("该手机已是您的认证手机");		
			                    $("#hintMobile").attr("class","hintError");
			                }else{
			                    $("#hintMobile").text("该手机已被注册认证，请重新输入");		
			                    $("#hintMobile").attr("class","hintError");
			                }
                        })
			        },
        ...
        checkSubmit = function(){
            ...
            $.ajax({
                ...
                dataType:"json",
                data:{CustID:custid,PhoneNum:phoneNum,PhoneCode:code,Type:"SetAuthenPhone",time:time.getTime()},
