...
namespace Linkage.BestTone.Interface.Utility
{
    ...
    /// 可以用户可以修改满足自己项目的需要。
    /// </summary>
    public abstract class OracleDBUtility
    {
        //数据库连接字符串(web.config来配置)，可以动态更改connectionString支持多数据库.		
        public static string connectionString = ConfigurationManager.AppSettings["OracleConStr"];

        public OracleDBUtility()
        {
        }
        #region 公用方法
        ...
        public static int GetMaxID(string FieldName, string TableName)
        {
            ...
            object obj = OracleDBUtility.GetSingle(strsql);
            if (obj == null)
            {
                return 1;
            }
        ...
        {
            object obj = OracleDBUtility.GetSingle(strSql, cmdParms);
            int cmdresult;
            if ((Object.Equals(obj, null)) || (Object.Equals(obj, System.DBNull.Value)))
            {
                cmdresult = 0;
        ...
        /// <param name="parameters">存储过程参数</param>
        /// <returns>OracleDataReader</returns>
        public static int RunProcedure(string storedProcName, IDataParameter[] parameters,out int result,out string msg)
        {
            result = -19999;
            msg = "";
            ...
            result = Convert.ToInt32((parameters[0].Value.ToString())); //接收函数返回值
